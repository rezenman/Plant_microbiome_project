...
echo "Analyzing sample: $input" >> log.txt
echo "Reference database: $db" >> log.txt
echo "Sample name: $sample_name" >> log.txt
echo "Clustering identity: $clustering_id" >> log.txt
...
cd-hit-est -i $input -o $out_file -c $clustering_id -n 10 -d 0 -M 0 -r 0 &>> log.txt

if [ $? -eq 0 ] ; then echo -e "Finished clustering: $sample_name" >> log.txt ; else echo "Failed clustering $sample_name, check log" >>log.txt && exit  ; fi
...
cd-hit-est-2d \
    -i $db \
    -i2 $out_file \
    -o $out_file_2d -c $clustering_id -n 10 -g 1 -d 0 -M 0 -r 0 -T 0 &>> log.txt

if [ $? -eq 0 ] ; then echo -e "Finished comparing: $sample_name to the database" >> log.txt ; else echo "Failed comparing $sample_name to the database, check log" >>log.txt && exit  ; fi

echo -e "All non-redundant sequences not clustered with the database are found in the file: $out_file_2d" >> log.txt
